class NoteForm extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
      <form id="noteForm">
        <h2>Create Note</h2>
        <input type="text" id="title" name="title" placeholder="Title" required pattern="^[a-zA-Z0-9 ._-]+$"
          minlength="8"
...
function validateTitle(event) {
  ...
  if (titleInput.validity.valueMissing) {
    ...
    titleInput.setCustomValidity('Minimal panjang title adalah delapan karakter.');
  } else if (titleInput.validity.patternMismatch) {
    titleInput.setCustomValidity('Tidak boleh diawali dengan simbol, mengandung white space atau spasi, dan mengandung karakter spesial seperti dolar ($).');
  }
